<g:if test="${mode=='edit' || taxonomy?.children?.size() > 0}">
	<fieldset>
		<legend class="text-primary clearfix padding-bottom">
			<g:message code="de.httc.plugin.taxonomy.terms" default="Terms" /><g:if test="${taxonomy?.children?.size()}"> <span class="badge middle">${taxonomy.termCount}</span></g:if>
			<g:if test="${mode=='edit'}">
		...
		<div id="taxonomy">
			<g:if test="${taxonomy?.children?.size() > 0}">
				<ul>

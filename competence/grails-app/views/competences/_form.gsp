...
<div class="form-group fieldcontain ${hasErrors(bean: competence, field: 'primaryTermId', 'error')} ">
	...
	<g:field type="text" name="primaryTermId" value="${competence?.primaryTermId}" class="form-control" />
</div>
<div class="form-group fieldcontain ${hasErrors(bean: competence, field: 'source', 'error')} ">
	...
	<g:select name="source" from="${de.httc.plugins.competence.Competence$Source?.values()}" keys="${de.httc.plugins.competence.Competence$Source.values()*.name()}" required="" value="${competence?.source?.name()}" class="form-control" />
</div>
